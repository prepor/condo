...
let handler monitoring request =
  let table = [
    "/" , string_response hello_handler
  ; "/v1/wait", string_response wait_handler
  (* ; "/v1/sse/watch", watch_handler *)
  ; "/v1/snapshot", string_response (snapshot_handler monitoring)
  ] in
...
let create monitoring port =
  server port (handler monitoring)
